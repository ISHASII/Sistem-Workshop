...
<div class="max-w-2xl mx-auto">
    ...
    <form id="profileForm" action="{{ route('admin.profile.update') }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end">
                <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 mr-2 bg-slate-100 text-slate-700 rounded-md">Batal</a>
        ...
        (function registerPasswordToggles(){
            function bindToggles() {
                document.querySelectorAll('.toggle-password').forEach(function(btn) {
                    // Avoid double-binding: remove previous handler marker
            ...
            // Also bind on DOMContentLoaded for normal flow
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', bindToggles);
            } else {
                // already ready — ensure bindings are present
                bindToggles();
            }
        })();
    </script>
    <script>
        // Prevent submitting without current password when changing to a new password
        (function(){
