...
@section('content')
  <div class="bg-white rounded-lg shadow p-6 w-full mx-auto">
    <h2 class="text-2xl font-semibold mb-4">Edit Performance</h2>

    <form action="{{ route('admin.performance.update', $performance) }}" method="POST" id="performance-form">
      @csrf
      @method('PUT')
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <select name="nrp" id="nrp" class="w-full border rounded px-3 py-2 @error('nrp') border-red-500 @enderror"
            required>
            <option value="">-- Pilih NRP --</option>
            @foreach($manpowers as $mp)
              <option value="{{ $mp->nrp }}" data-nama="{{ $mp->nama }}" {{ (old('nrp', $performance->manpower->nrp ?? '') == $mp->nrp) ? 'selected' : '' }}>
                {{ $mp->nrp }} - {{ $mp->nama }}
              </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <input type="text" id="nama" class="w-full border rounded px-3 py-2 bg-slate-50"
            value="{{ $performance->manpower->nama ?? '' }}" readonly>
        </div>
        <div>
          ...
          <select name="job_order_id"
            ...
            @foreach($joborders as $jo)
              <option value="{{ $jo->id }}" {{ (old('job_order_id', $performance->job_order_id) == $jo->id) ? 'selected' : '' }}>
                #{{ $jo->id }} - {{ $jo->project }}
              </option>
            @endforeach
          </select>
      ...
      <div class="border rounded p-4 mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          @php
            $selectedIds = old('checklist_quality_item_ids', $performance->checklistQualityItems->pluck('id')->all());
          @endphp
          @forelse($checklistItems as $item)
            <label class="flex items-center space-x-2 p-2 rounded hover:bg-slate-50">
              <input type="checkbox" name="checklist_quality_item_ids[]" value="{{ $item->id }}" class="check-item" {{ in_array($item->id, $selectedIds) ? 'checked' : '' }}>
              <span>{{ $item->name }}</span>
            </label>
          @empty
            <div class="text-sm text-slate-500">Checklist kualitas belum tersedia. Silakan tambah di menu Master Data.</div>
          @endforelse
        </div>
      </div>
      ...
      <div class="flex items-center justify-between mt-4">
        <div>
          ...
          <span id="score-preview" class="font-semibold">{{ $performance->score }}%</span>
        </div>
        <div class="space-x-2">
