  ...
  <form action="{{ route('admin.performance.update', $performance) }}" method="POST" id="performance-form">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      ...
      <div>
        ...
        <input type="text" id="nama" class="w-full border rounded px-3 py-2 bg-slate-50" value="{{ $performance->manpower->nama ?? '' }}" readonly>
      </div>
      <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        @foreach($checks as $name => $label)
          <label class="flex items-center space-x-2 p-2 rounded hover:bg-slate-50">
            <input type="checkbox" name="{{ $name }}" value="1" class="check-item" {{ old($name, $performance->{$name}) ? 'checked' : '' }}>
            <span>{{ $label }}</span>
          </label>
        @endforeach
      </div>
