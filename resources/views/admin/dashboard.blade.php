...
<div class="w-full min-h-screen py-4 px-2 sm:py-6 sm:px-4 md:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-green-200/50 p-3 transform hover:scale-[1.005] transition-all duration-300 hover:shadow-2xl">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                    <div class="flex items-center gap-2">
                        <div class="w-9 h-9 bg-gradient-to-br from-green-100 to-emerald-100 rounded-xl flex items-center justify-center shadow-sm">
                    ...
                    <form id="filterJobOrderMonth" class="flex flex-wrap items-center gap-2 max-w-full sm:max-w-xs" method="GET" action="">
                        <select name="bulan" class="w-28 sm:w-auto border border-gray-300/50 bg-white/80 rounded-xl px-2 py-1 text-xs focus:ring-2 focus:ring-green-500 transition-all truncate">
                        ...
                        <select name="tahun" class="w-20 sm:w-auto border border-gray-300/50 bg-white/80 rounded-xl px-2 py-1 text-xs focus:ring-2 focus:ring-green-500 transition-all truncate">
                            @foreach(range(now()->year-5, now()->year+1) as $y)
                                <option value="{{ $y }}" @if($y == request('tahun', now()->year)) selected @endif>{{ $y }}</option>
                            @endforeach
                        </select>
                        <button type="submit" class="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-3 py-1 rounded-xl text-xs font-semibold whitespace-nowrap flex-shrink-0">
                            <span class="hidden sm:inline">Tampilkan</span>
                            <svg class="sm:hidden w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10h18v8a1 1 0 01-1 1H4a1 1 0 01-1-1v-8z"></path></svg>
                        </button>
                    </form>
                </div>

                @php $jobMonthlyScrollable = isset($joborders_monthly) && count($joborders_monthly) > 6; @endphp
                <div class="overflow-x-auto rounded-xl border border-gray-200/50 {{ $jobMonthlyScrollable ? 'max-h-56 overflow-y-auto' : '' }}">
                        ...
                        <tbody class="divide-y divide-gray-200/50">
                            @forelse($joborders_monthly as $jo)
                                <tr class="hover:bg-green-50/50 transition-all duration-200">
                                    <td class="px-3 py-2 font-semibold text-blue-700">{{ $jo->start ? \Carbon\Carbon::parse($jo->start)->format('d M Y') : '-' }}</td>
                                    <td class="px-3 py-2 text-gray-700 max-w-xs truncate">{{ $jo->project }}</td>
                                    <td class="px-3 py-2 text-gray-700">{{ $jo->evaluasi ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center py-6 text-gray-500">Tidak ada job order bulan ini.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-red-200/50 p-3 transform hover:scale-[1.005] transition-all duration-300 hover:shadow-2xl">
                <div class="flex items-center gap-2 mb-3">
                    <div class="w-9 h-9 bg-gradient-to-br from-red-100 to-rose-100 rounded-xl flex items-center justify-center shadow-sm">
                    ...
                    <div class="w-full p-2">
                        <table class="w-full text-xs">
                            ...
                            <tbody class="divide-y divide-gray-200/50">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-6 text-gray-500">Tidak ada material kritis.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-purple-200/50 p-4 sm:p-6 transform hover:scale-[1.005] transition-all duration-300 hover:shadow-2xl">
                ...
                <button id="mat-prev-btn" type="button" class="text-xs px-2 py-1 rounded-xl bg-gray-100 hover:bg-gray-200 border border-gray-200 flex-shrink-0">
                    <span class="hidden sm:inline">Prev</span>
                        <svg class="sm:hidden w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                </button>
                ...
                <button id="mat-play-btn" type="button" class="text-xs px-2 py-1 rounded-xl bg-gray-100 hover:bg-gray-200 border border-gray-200 flex-shrink-0" aria-pressed="true">
                    <span class="hidden sm:inline mat-play-text">Pause</span>
                        <svg class="sm:hidden w-4 h-4 inline ml-1" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <g class="icon-pause"><rect x="6" y="5" width="4" height="14" rx="1"></rect><rect x="14" y="5" width="4" height="14" rx="1"></rect></g>
                            <g class="icon-play" style="display:none;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v18l15-9L5 3z"></path></g>
                        </svg>
                </button>
                ...
                <button id="mat-next-btn" type="button" class="text-xs px-2 py-1 rounded-xl bg-gray-100 hover:bg-gray-200 border border-gray-200 flex-shrink-0">
                    <span class="hidden sm:inline">Next</span>
                        <svg class="sm:hidden w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                </button>
