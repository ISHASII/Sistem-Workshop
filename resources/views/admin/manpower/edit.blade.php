    ...
    <div class="max-w-4xl mx-auto space-y-4 md:space-y-6 p-4">
        ...
        <div class="flex items-center gap-3 md:gap-4">
            <a href="{{ route('admin.manpower.index') }}"
                class="p-2 hover:bg-slate-100 rounded-lg transition-colors duration-150 flex-shrink-0">
                <svg class="w-5 h-5 md:w-6 md:h-6 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
            </a>
            <div class="min-w-0">
                <h1 class="text-xl md:text-3xl font-bold text-slate-800">Edit Data Manpower</h1>
                <p class="text-xs md:text-sm text-slate-600 mt-1 truncate">Perbarui informasi: <span
                        class="font-semibold text-rose-600">{{ $manpower->nama }}</span></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-rose-50 to-red-50 px-4 md:px-6 py-3 md:py-4 border-b border-rose-100">
                <div class="flex items-center gap-2 md:gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xs md:text-sm text-slate-600 hidden sm:block">Perbarui data identitas dan status
                            karyawan</p>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Body -->
            <form action="{{ route('admin.manpower.update', $manpower) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 md:p-4">
                    <div class="flex items-start gap-2 md:gap-3">
                        <svg class="w-4 h-4 md:w-5 md:h-5 text-amber-600 mt-0.5 flex-shrink-0" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs md:text-sm text-amber-700 mt-1">Pastikan perubahan data sudah sesuai sebelum
                                menyimpan. Data yang sudah diupdate tidak dapat dikembalikan secara otomatis.</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Photo Upload -->
                <div>
                    <label for="photo" class="block text-xs md:text-sm font-semibold text-slate-700 mb-2">Foto
                        (opsional)</label>
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3 md:gap-4">
                        ...
                        <div id="photo-preview-wrap"
                            class="w-16 h-16 md:w-20 md:h-20 rounded-md overflow-hidden border hidden flex-shrink-0">
                            <img id="photo-preview-img"
                                src="@if($manpower->photo){{ asset('storage/' . $manpower->photo) }}@endif"
                                alt="Foto {{ $manpower->nama }}" class="w-full h-full object-cover"
                                data-original="@if($manpower->photo){{ asset('storage/' . $manpower->photo) }}@endif" />
                        </div>
                        <div id="photo-preview-controls">
                            <button id="photo-clear-btn"
                                class="inline-flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1.5 text-xs md:text-sm bg-white border border-slate-200 text-red-600 rounded-lg shadow-sm hover:bg-slate-50">Hapus
                                preview</button>
                        </div>
                    </div>
                    @error('photo')
                ...
                <!-- Form Actions -->
                <div
                    class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 md:gap-3 pt-3 md:pt-4 border-t border-slate-200">
                    <button type="submit"
                        class="inline-flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 text-sm md:text-base bg-gradient-to-r from-rose-600 to-red-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl hover:from-rose-700 hover:to-red-700 transition-all duration-200">
                    ...
                    <a href="{{ route('admin.manpower.index') }}"
                        class="inline-flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 text-sm md:text-base bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-semibold transition-colors duration-200">
                        <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
