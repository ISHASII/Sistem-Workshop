    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-4 sm:py-8">
        <div class="max-w-4xl mx-auto px-2 sm:px-4">
            ...
            <div class="bg-white rounded-2xl shadow-xl shadow-slate-900/5 border border-slate-200/50 overflow-hidden">
                ...
                <!-- Form Body -->
                <form action="{{ route('customer.joborder.update', $joborder->id) }}" method="POST" class="p-8"
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-slate-200">
                        ...
                        <div class="flex items-center space-x-3">
                            <a href="{{ route('customer.joborder.index') }}" id="cancelBtn" data-swal-cancel
...
<style>
    /* Custom styling for booked ranges in flatpickr (keep separate from generic disabled days) */
...
<script>
    // Debug: Check if joborder data is available
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const bookedDateRanges = rawBooked.map(function (r) {
            try {
                var pf = String(r.from).split('-');
                var pt = String(r.to).split('-');
                var fd = new Date(parseInt(pf[0], 10), parseInt(pf[1], 10) - 1, parseInt(pf[2], 10));
                var td = new Date(parseInt(pt[0], 10), parseInt(pt[1], 10) - 1, parseInt(pt[2], 10));
                td.setHours(23, 59, 59, 999);
                return { from: fd, to: td };
            } catch (e) { return null; }
        }).filter(function (x) { return !!x; });
        // Pass Date ranges to flatpickr as Date objects for more reliable disabling
        ...
        document.querySelectorAll('.datepicker').forEach(function (el) {
            ...
            let parsedDate = null;
            if (rawDate) {
                // Use a robust parser (handles datetimes and ISO formats)
            ...
            var disableFn = function (date) {
                try {
                    var dd = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
                    for (var i = 0; i < bookedDateRanges.length; i++) { var br = bookedDateRanges[i]; if (br && br.from && br.to && dd >= br.from.getTime() && dd <= br.to.getTime()) return true; }
                } catch (e) { return false; }
                return false;
            };
            const opts = {
                // Display dates in UI as d-m-Y for consistent UX with create view
                dateFormat: 'd-m-Y',
                // robust disable using precomputed ranges and fallback for generated disabled ranges
            ...
            if (el.name === 'start') {
                ...
                el.addEventListener('blur', function () {
                    try {
                        ...
                        for (var oi = 0; oi < bookedDateRanges.length; oi++) { var b = bookedDateRanges[oi]; if (b && b.from && b.from.getTime() > t) { ns = b.from; break; } }
            ...
            // Attach click handler to prevent selecting booked days
            if (fp && fp.calendarContainer) {
                fp.calendarContainer.addEventListener('click', function (ev) {
                ...
                fp.calendarContainer.addEventListener('keydown', function (ev) {
            ...
            // If this is the start picker, update end picker constraints when changed
            if (el.name === 'start') {
                fp.config.onChange.push(function (selectedDates) {
                    try {
                        ...
                        // find next booked start after sel
                        var ns = null;
                        var selTime = new Date(sel.getFullYear(), sel.getMonth(), sel.getDate()).getTime();
                        for (var iii = 0; iii < bookedDateRanges.length; iii++) { var b = bookedDateRanges[iii]; if (b && b.from && b.from.getTime() > selTime) { ns = b.from; break; } }
                        if (ns) {
            ...
            // parse a date string in d-m-Y, Y-m-d, or datetime formats to Date object at midnight
            function parseDateInput(str) {
                if (!str) return null;
                str = String(str).trim();
                // strip time portion if present
                var bare = str.split(' ')[0].split('T')[0];
                if (/^\d{2}-\d{2}-\d{4}$/.test(bare)) {
                    const [dd, mm, yyyy] = bare.split('-');
                    return new Date(parseInt(yyyy, 10), parseInt(mm, 10) - 1, parseInt(dd, 10));
                }
                if (/^\d{4}-\d{2}-\d{2}$/.test(bare)) {
                    const [yyyy, mm, dd] = bare.split('-');
                    return new Date(parseInt(yyyy, 10), parseInt(mm, 10) - 1, parseInt(dd, 10));
                }
            ...
            function getMinDateForFp(el) {
                if (!el) return null;
                // allow start to navigate earlier months/dates; constrain end to the chosen start or earliest
                ...
                if (el.name === 'end') {
                    ...
                    // allow end to be navigated freely unless start selected
                    return null;
                }
                return null;
            }

            // update nav controls to prevent going to months earlier than minDate
