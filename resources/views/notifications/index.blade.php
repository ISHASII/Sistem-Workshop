...
@section('content')
    <div class="space-y-6">
                ...
                <div class="flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-600 mt-0.5">{{ $pageSubtitle }}</p>
                        </div>
                    </div>
                    @if($unreadCount > 0)
                        <button onclick="markAllAsRead()" class="inline-flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl hover:bg-red-700 transition-all duration-200 focus:outline-none">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
            ...
            @if($notifications->count() > 0)
                                    ...
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2">
                                                <h3 class="text-sm font-semibold text-slate-800 mb-1">{{ $notification->title }}</h3>
                                                @if($type === 'job_order_resubmitted')
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 text-xs font-semibold">Resubmisi</span>
                                                @endif
                                            </div>
                                            ...
                                            @if($notification->actionBy)
                                                <p class="text-xs text-slate-500">By: {{ $notification->actionBy->name }} {{ $notification->actionBy->npk ? '(' . $notification->actionBy->npk . ')' : '' }}</p>
                                            @endif
                                        </div>
                                        ...
                                        <div class="flex flex-col items-end gap-2 ml-4">
                                            <div class="flex items-center gap-2">
                                                @if($notification->jobOrder)
                                                    <a href="{{ route($showRouteName, $notification->id) }}" class="inline-flex items-center gap-1 px-2 py-1 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg transition-colors duration-200 text-xs font-medium">
                ...
                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100">{{ $notifications->links() }}</div>
            @else
    ...
    <script>
        const markAsReadUrlTemplate = @json(route($markAsReadRouteName, ['notification' => '__ID__']));
        const markAllReadUrl = @json(route($markAllReadRouteName));

        function markAsRead(notificationId) {
            const url = markAsReadUrlTemplate.replace('__ID__', notificationId);
            fetch(url, {
                method: 'POST',
                headers: {
        ...
        function markAllAsRead() {
            Swal.fire({
                ...
                customClass: {
                    confirmButton: 'inline-flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-xl font-semibold shadow-lg hover:bg-red-700 focus:outline-none',
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({ title: 'Error!', text: data.message || 'Failed to mark notifications as read', icon: 'error' });
                        }
                    })
                    .catch(error => {
